...
st.markdown("""
<style>
    /* ── Main title: inherits theme colour so it works in light AND dark ── */
    .main-title {
        font-size: 2.4rem;
        font-weight: 800;
    ...
    /* ── Verdict banners: always white text on coloured background ── */
    .verdict-fake {
    ...
    /* ── Metric cards: transparent background so they inherit the sidebar
          theme colour; explicit dark text with !important so it overrides
          Streamlit's dark-mode text styles ── */
    .metric-card {
    ...
    /* ── Sidebar: force light background with dark text
          so metric cards are always readable ── */
    ...
    [data-testid="stSidebar"] .metric-card b {
...
with st.sidebar:
    ...
    for k, v in {
        ...
        st.markdown(
            ...
            f"<b style='color:#5dade2;'>{k}</b>"
            f"<span style='color:#e8eaf0;'>: {v}</span>"
            f"</div>",
            unsafe_allow_html=True
        )
